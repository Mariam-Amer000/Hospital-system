...
// add a patient .......
    void hospital::addPatient() {
        ...
        cout << "1. القلب (Cardiology)\n";
        cout << "2. الأعصاب (Neurology)\n";
        cout << "3. العظام (Orthopedics)\n";
        cout << "4. الأطفال (Pediatrics)\n";
        cout << "5. الجلدية (Dermatology)\n";
        cout << "6. الجهاز الهضمي (Gastroenterology)\n";
        cout << "7. الأشعة (Radiology)\n";
        cout << "8. الكلى (Nephrology)\n";
        cout << "9. جراحة القلب والصدر (Cardiothoracic Surgery)\n";
        cout << "10. العيون (Ophthalmology)\n";
        cout << "11. الطب النفسي (Psychiatry)\n";
    ...
    cout << "Invalid choice. Please select a valid clinic." << endl;
        return;
         ...
         string selectedSpec = doctor->specializations[choice - 1];
      ...
      if (doctor->getSpecialization() == selectedSpec && doctor->Appointments()) { 
        availableDoctors.push_back(doctor);
         ...
         << " (Patients: " << availableDoctors[i]->patient_number << ")\n";
            }
    ...
    void hospital::addDoctor() {
        ...
        dataManager.listOfEmployee.push_back(d);
        dataManager.saveData();
    }
    ...
    void hospital::addStaff() {
        ...
        dataManager.listOfEmployee.push_back(s);
        dataManager.saveData();
    }
    ...
    void hospital::updatePatient(int id)
    {
        ...
        for (auto &patient : listOfPatient)
        {
            if (patient.getId() == id)
            {
                int choice;
   ...
   {
       dataManager.loadData();
       for (const auto &p : dataManager.listOfPatients)
       {
           if (p->getId() == id)
           {
